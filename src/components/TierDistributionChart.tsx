import type { GroupAdmin, ServiceTierName } from '@/features/platform-admin/types'
...
// (jumlah GA diharapkan kecil, lihat komentar api.ts).
//
...
// H6 ini -- cuma 3 kategori tetap (starter/business/enterprise), jadi
// stroke-dasharray sudah cukup, tanpa dependency baru. Ganti ke ECharts
// begitu npm install berhasil ATAU begitu chart pertama yang butuh
// interaktivitas sungguhan (Gantt/Workload, S5+) mulai dikerjakan --
// jangan biarkan substitusi ini jadi pola permanen untuk chart lain.
const TIER_COLOR: Record<ServiceTierName, string> = {
  enterprise: 'oklch(0.78 0.12 165)',
  business: 'oklch(0.72 0.15 25)',
  starter: 'oklch(0.55 0.01 70)',
}

const TIER_LABEL: Record<ServiceTierName, string> = {
...
export function aggregateByTier(groupAdmins: GroupAdmin[]): Record<ServiceTierName, number> {
  return groupAdmins.reduce<Record<string, number>>((acc, ga) => {
    if (!ga.tier) return acc
    acc[ga.tier] = (acc[ga.tier] ?? 0) + 1
    return acc
  }, {}) as Record<ServiceTierName, number>
}
...
export default function TierDistributionChart({ groupAdmins }: { groupAdmins: GroupAdmin[] }) {
  ...
  const tiers = Object.keys(counts) as ServiceTierName[]
  ...
  const segments = tiers.map((tier) => {
    ...
    const segment = { tier, value, length, offset }
    offset += length
    return segment
  })
    ...
    <div className="border border-pa-border p-3">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          {segments.map(({ tier, value }) => (
            <div key={tier} className="flex items-center gap-2 font-mono text-[10.5px]">
              <span className="inline-block h-2.5 w-2.5" style={{ backgroundColor: TIER_COLOR[tier] }} />
              <span className="text-text-muted">{TIER_LABEL[tier]}</span>
